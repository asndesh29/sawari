...
import AddBrandProductForm from './addBrandProductForm';
...
class BrandList extends React.Component {
  ...
  updateBrand = (id) => {
    ...
    const brandToUpdate = main.initialAdminData.vehicleBrandProduct.find(vb => vb.id === id);
    updateFormValue('addProduct', { ...main.initialAdminData.vehicleBrandProduct, ...brandToUpdate });
  ...
  mainDelete = async () => {
    ...
    const res = await deleteProduct(showDelete);
    if (res) {
  ...
  render() {
    ...
    return (
      <div className="admin-add-brand-content-table">
        <div style={{ width: '100%' }}>
          {main.initialAdminData.vehicleBrandProduct && (
            <Table
              headers={{ id: 'Id', name: 'Name', price: 'Price', image: 'Image', update: 'Update', delete: 'Delete' }}
              data={main.initialAdminData.vehicleBrandProduct.filter(vp => (parseInt(vp.stypeId, 10) === parseInt(form.addProduct.stypeId, 10) && parseInt(vp.sbId, 10) === parseInt(form.addProduct.sbId, 10))).map(obj => (
                {
                  ...obj,
