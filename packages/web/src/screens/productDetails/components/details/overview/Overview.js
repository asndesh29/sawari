...
const iconAndUnitHandler = (key) => {
  switch (key) {
    case 'mileage':
      return { icon: <IoMdSpeedometer size={30} color="#DBDBDB" />, unit: 'Km/Kg', preUnit: '' };
    case 'engine':
      return { icon: <GiVendingMachine size={30} color="#DBDBDB" />, unit: 'cc', preUnit: '' };
    case 'BHP':
      return { icon: <FaBolt size={30} color="#DBDBDB" />, unit: '', preUnit: '' };
    case 'transmission':
      return { icon: <FaTransgender size={30} color="#DBDBDB" />, unit: '', preUnit: ''};
    case 'seats':
      return { icon: <MdAirlineSeatLegroomExtra size={30} color="#DBDBDB" />, unit: '', preUnit: '' };
    case 'serviceCost':
      return { icon: <GiMoneyStack size={30} color="#DBDBDB" />, unit: '/yr', preUnit: 'NRs.' };
    default:
...
const overviewElement = (label, key, obj) => {
  return (
    <div className="overview-element">
      ...
      <span style={{ color: '#DBDBDB', marginBottom: 5 }}>{label}</span>
