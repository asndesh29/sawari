    ...
    upload(req, res, async (err) => {
      ...
      if (req.file) {
        ...
        if (userValidation) {
          ...
          const updateRes = await addBrand({ ...req.body, image: req.file.filename, userId: userValidation.id });
          if (updateRes) {
            res.statusCode = 200;
        ...
        if (userValidation) {
          delete req.body.token;
          const updateRes = await addBrand({ ...req.body, userId: userValidation.id });
          if (updateRes) {
            res.statusCode = 200;
            res.send(JSON.stringify(updateRes));
          } else {
