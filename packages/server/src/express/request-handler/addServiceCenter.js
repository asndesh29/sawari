...
import addServiceCenter from '../../api/addServiceCenter';
...
export default async (req, res) => {
  console.log('add ServiceCenter called');
  try {
    upload(req, res, async (err) => {
      ...
      if (req.file) {
        ...
        if (userValidation) {
          ...
          const updateRes = await addServiceCenter({ ...req.body, image: req.file.filename, userId: userValidation.id });
          if (updateRes) {
            res.statusCode = 200;
        ...
        if (userValidation) {
          delete req.body.token;
          const updateRes = await addServiceCenter({ ...req.body, userId: userValidation.id });
          if (updateRes) {
            res.statusCode = 200;
            res.send(JSON.stringify(updateRes));
          } else {
